...
def index():
    ...
    return render_template("main.html", stocks=stocks, cash=round(cash, 2), total=round(total, 2))
...
def buy():
    ...
    if request.method == "POST":
        ...
        #Updating row into the stock table
        else:
        ...
        # Redirecting user to index page with a success message
        flash("Bought!")
        return redirect("/")
...
def register():
    ...
    # User reached route via POST (as by submitting a form via POST)
    if request.method == "POST":
        # Error-checking for username submission
        if not request.form.get("username"):
            return apology("sorry, please provide username", 403)

        # Error-checking for password submission
        elif not request.form.get("password"):
            return apology("sorry, please provide password", 403)

        # Error-checking for password match
        elif request.form.get("password") != request.form.get("confirm-password"):
            return apology("sorry, the passwords don't match", 403)

        # Error-checking and query match if username exists
